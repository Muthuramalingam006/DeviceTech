...
public class GeneralFunctionalities {
	...
	def validateBackToTopButton() {
		...
		WebUI.delay(5)
		WebUI.scrollToPosition(0,1200)//WebUI.executeJavaScript("window.scrollTo(0, 1500);", null)
		WebUI.delay(3)
		...
		WebUI.delay(10)// Step 3: Verify presence and visibility
		if(WebUI.waitForElementPresent(topButton, 10)){
			if(WebUI.verifyElementVisible(topButton)){
				...
				WebUI.executeJavaScript("arguments[0].click();",
						Arrays.asList(topButtonElement))
				WebUI.delay(2)
	...
	def validatereCaptcha() {
		...
		/*WebUI.verifyElementPresent(findTestObject("IphoneDevice/Hamburger"),5)
		 ...
		 WebUI.verifyElementPresent(findTestObject("IphoneDevice/Menu_items/contact_us"),2)
		 WebUI.delay(3)*/WebUI.click(findTestObject("IphoneDevice/Menu_items/contact_us"))

		//WebUI.click(findTestObject("ContactUs/CONTACT_US"))
		/*	WebUI.waitForElementVisible(findTestObject('General/Captcha'), 10)
		 WebUI.scrollToElement(findTestObject('General/Captcha'), 5)
		 WebUI.mouseOver(findTestObject('General/Captcha'))*//*
		 // Define iframe
		 TestObject captchaIframe = new TestObject()
		 captchaIframe.addProperty("xpath", ConditionType.EQUALS, "General/Captcha")
		 // Define CAPTCHA icon
		 TestObject captchaIcon = new TestObject()
		 captchaIcon.addProperty("xpath", ConditionType.EQUALS, "General/CaptchaIcon")
		 // Switch to iframe
		 ...
		 WebUI.verifyElementPresent(findTestObject("General/CaptchaHeading"), 2)
		 WebUI.verifyElementPresent(findTestObject("General/CaptchaText"), 2)
		 WebUI.verifyElementPresent(findTestObject("General/TakeAct"), 2)*/
