...
public class OurBrands {
	...
	def validateOurBrands() {
		...
		for (List group : brandChecks) {
			...
			WebUI.delay(4)
			WebUI.click(findTestObject("Homepage/OurBrandsNZ/" + letter))
			WebUI.delay(4)

			for (String brand : brands) {
				TestObject obj = findTestObject("Homepage/OurBrandsNZ/" + brand)
				WebUI.delay(2)
	...
	def validateBrandDetailsPage() {
		...
		if (WebUI.verifyElementPresent(findTestObject("Homepage/OurBrandsNZ/MedelaHeading"), 5)) {
			// Get the text of the element
			String headingText = WebUI.getText(findTestObject("Homepage/OurBrandsNZ/MedelaHeading"))
	...
	def validateDownloadBrochure() {
		...
		String pageTitle = WebUI.getWindowTitle()
		println("📄 Page title is: " + pageTitle)
		WebUI.navigateToUrl("https://w3q.device.com.au/our-brands/Ambu")
